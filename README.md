...
- **Interactive** (default in terminal) — one slot row per child of the target directory, descended through any single-child chain so rows split where the suite actually branches: with `a/ra/…` and `a/rb/…`, `tstr run a` shows rows `ra` and `rb`, while `tstr run a/ra` (only one path down) lists each test in the leaf as its own row. Each row ends with its `done/total` counter and, once its first result lands, its wall-clock time so far (frozen when the row completes). Per-test glyphs (`✓✗-·`) when there's room, or a colored bucketed bar otherwise — gradient hue from green (all pass) through yellow (skip-leaning) to red (all fail). `--display=bars` forces bars on short rows. Under sequential `--repeat`, the slots reset to all-pending at the start of each pass (so every iteration animates fresh) and the status line carries an `Iter k/N` marker. Under concurrent `--repeat`, each directory is a single bucketed bar spanning all its `tests × repeat` cells, filling as the overlapping passes complete.
