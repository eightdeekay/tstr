...
4. **`cargo test`** — must pass. This compiles the crate, which rewrites the
   `name = "tstr"` line in `Cargo.lock` to the new version. Do **not** run a
   separate `cargo build`; the test run already syncs the lock. If tests fail,
   stop and report — do not commit.
5. **Stage** — `git add -A` (Cargo.toml, Cargo.lock, CHANGELOG.md, any
   UPGRADING.md/codemod, and the source change all ride in one commit).
6. **Commit** — use the new CHANGELOG entry as the message: first line is the
   `## [x.y.z] - date` heading, body is the rest of the entry. End the message
   with:
   `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>`
7. **Tag** — `git tag -a vX.Y.Z -m "vX.Y.Z"` matching the bumped version.
8. **Push** — `git push --follow-tags` to the current branch (so the commit and
   its tag go together).
